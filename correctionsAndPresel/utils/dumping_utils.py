...
# Ensure that variables from flow_inputs are kept
skip_photon_fields = [
    "seediEtaOriX",
    "cutBased",
    "hasConversionTracks",
...
skip_photon_fields = [item for item in skip_photon_fields if item not in flow_inputs]
...
def diphoton_list_to_pandas(self, diphotons: ak.Array) -> pandas.DataFrame:
    ...
    for key, value in output.items():
        try:
            ...
            logger.warning(f"FAILED COLUMN: {key}")
            logger.warning(f"dtype = {value.dtype}")
            logger.warning(f"type = {type(value)}")
            raise
...
def ps_list_to_pandas(pseudos: ak.Array) -> pandas.DataFrame:
    results = []
    for field in ak.fields(pseudos):
        if field not in [
                "mass_gggg",
                "pT1_ma1",
                "pT2_ma1",
                ...
                "genWeight",
                "BDT_score"]:
            for subfield in ak.fields(pseudos[field]):
                try:
                    if subfield not in skip_photon_fields:
                        try:
                            if ((field == "LeadPs" or field == "SubleadPs") or (field == "gen_LeadPs" or field == "gen_SubleadPs")) and (subfield == "leading_pho" or subfield == "subleading_pho"):
                                for subsubfield in ak.fields(pseudos[field][subfield]):
                                    if subsubfield not in skip_photon_fields:
                                        sub = f"{field}_{subfield}_{subsubfield}"
                                        results.append(
                                            pandas.Series(ak.to_numpy(pseudos[field][subfield][subsubfield][:,0]), name=sub)
                                        )

                            if subfield != "leading_pho" and subfield != "subleading_pho":
                                sub = f"{field}_{subfield}"
                                results.append(
                                    pandas.Series(ak.to_numpy(pseudos[field][subfield][:,0]), name=sub)
                                )
                        except:
            ...
            try:
                results.append(
                    pandas.Series(ak.to_numpy(pseudos[field][:,0]), name=field.replace("/","_"))
                )
            except:
