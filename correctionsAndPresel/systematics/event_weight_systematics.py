...
def SF_photon_ID(
    ...
    if "2024" in year:
        if is_correction:
            ...
            sf = np.ones(len(weights._weight))
            #logger.warning(photons["masspoint"][0][0])
            #logger.warning("abive value is masspoint test")
            if photons["masspoint"][0][0] == 15.0:
                sfup = 0.42/100
                sfdown = 2.12/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 20.0:
                sfup = 0.51/100
                sfdown = 2.34/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 25.0:
                sfup = 0.94/100
                sfdown = 2.26/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 30.0:
                sfup = 2.23/100
                sfdown = 2.76/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 35.0:
                sfup = 0.78/100
                sfdown = 3.30/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 40.0:
                sfup = 1.06/100
                sfdown = 3.54/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 45.0:
                sfup = 1.35/100
                sfdown = 3.72/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 50.0:
                sfup = 1.66/100
                sfdown = 3.94/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 55.0:
                sfup = 1.66/100
                sfdown = 3.81/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
            elif photons["masspoint"][0][0] == 60.0:
                sfup = 1.59/100
                sfdown = 3.80/100
                sfup = np.ones(len(weights._weight))*sfup
                sfdown = np.ones(len(weights._weight))*sfdown
    #logger.warning(f"AT PHOTON ID SCALEFACTORS --- MASSPOINT : {photons['masspoint']}")
    #sf = np.ones(len(weights._weight))
    #logger.warning(type(sfup))
...
def ElectronVetoSF(photons, weights, year="2017", is_correction=True, **kwargs):
    ...
    if year in ["2016", "2017", "2018"]:
        ...
        if is_correction:
            ...
            sf_subsublead = evaluator.evaluate(
                abs(photons["pho_subsublead"].ScEta), photons["pho_subsublead"].r9, "nominal"
            )
            sf_subsubsublead = evaluator.evaluate(
                abs(photons["pho_subsubsublead"].ScEta), photons["pho_subsubsublead"].r9, "nominal"
            )
            sf = sf_lead * sf_sublead * sf_subsublead * sf_subsubsublead
            ...
            _sf = sf_lead * sf_sublead * sf_subsublead * sf_subsubsublead
            ...
            unc_subsublead = evaluator.evaluate(
                abs(photons["pho_subsublead"].ScEta), photons["pho_subsublead"].r9, "uncertainty"
            )
            unc_subsubsublead = evaluator.evaluate(
                abs(photons["pho_subsubsublead"].ScEta), photons["pho_subsubsublead"].r9, "uncertainty"
            )

            sfup = (sf_lead + unc_lead) * (sf_sublead + unc_sublead) * (sf_subsublead + unc_subsublead) * (sf_subsubsublead + unc_subsubsublead) / _sf
            sfdown = (sf_lead - unc_lead) * (sf_sublead - unc_sublead) * (sf_subsublead - unc_subsublead) * (sf_subsubsublead - unc_subsubsublead) / _sf

    name = "ElectronVetoSF_corr" if is_correction else "ElectronVetoSF"
    weights.add(name=name, weight=sf, weightUp=sfup, weightDown=sfdown)
    '''
    logger.warning("Elevtron Veto")
    logger.warning("Photon pt:")
    for i in range(1, 5):
        logger.warning(f"Photon {i}: {ak.to_list(photon_pt[i][38])}")
    
    logger.warning("\nPhoton eta:")
    for i in range(1, 5):
        logger.warning(f"Photon {i}: {ak.to_list(photon_eta[i][38])}")
    
    logger.warning("\nPhoton r9:")
    for i in range(1, 5):
        logger.warning(f"Photon {i}: {ak.to_list(photon_r9[i][38])}")
    '''
    return weights
...
def PreselSF(photons, weights, year="2017", is_correction=True, **kwargs):
    ...
    if year in ["2016", "2017", "2018"]:
        ...
        # Forcing low mass SFs for this analysis
        # json_file = os.path.join(os.path.dirname(__file__), "JSONs/Preselection/2022/Preselection_2022PreEE.json")
        json_file = os.path.join(os.path.dirname(__file__), "JSONs/Preselection/2022/Preselection_2022PostEE_LowHgg.json")
        logger.warning("Using low mass H->gg SFs for this analysis!")
    elif year == "2022postEE":
        # Forcing low mass SFs for this analysis
        # json_file = os.path.join(os.path.dirname(__file__), "JSONs/Preselection/2022/Preselection_2022PostEE.json")
        json_file = os.path.join(os.path.dirname(__file__), "JSONs/Preselection/2022/Preselection_2022PreEE_LowHgg.json")
        logger.warning("Using low mass H->gg SFs for this analysis!")
    elif year == "2023preBPix":
    ...
    # For 2024 use 2023postBPix SFs for now. This is only a placeholder until 2024 SFs are available!
        ...
        #json_file = os.path.join(os.path.dirname(__file__), "JSONs/Preselection/2023postBPix/Preselection_2023PostBPiX.json")
        json_file = os.path.join(os.path.dirname(__file__), "JSONs/Preselection/2023postBPix/Preselection_2023PostBPix_LowHgg.json")
        logger.warning("Using 2023postBPix Preselection SFs for 2024 as a placeholder until 2024 SFs are available!")

        """
        json_file = os.path.join(os.path.dirname(__file__), "JSONs/Preselection/2023preBPix/Preselection_2023PreBPiX_LowHgg.json")
        logger.warning("Using 2023preBPix Preselection SFs for 2024 as a placeholder until 2024 SFs are available! These NTuples cannot be used for a final physics result.")
        """
...
#Helper functions for triggerSF - By Tanay
def term_maker(n):
    final_terms = []

    for term in itertools.product(["s", "u", "n"], repeat=n):
        count_s = term.count("s")
        count_u = term.count("u")

        if count_s >= 1 and (count_s + count_u) >= 2:
            final_terms.append(term)

    return final_terms

FINAL_TRIGGER_TERMS = term_maker(4)
...
def TriggerSF(photons, weights, year="2017", is_correction=True, **kwargs):
    ...
    if year in ["2016", "2017", "2018", "2023preBPix", "2023postBPix", "2024"]:
        if year == "2024":
            json_file_lead = os.path.join(os.path.dirname(__file__), f"JSONs/TriggerSF/2023postBPix/TriggerSF_lead_2023postBPix_LowHgg_asym.json")
            json_file_sublead = os.path.join(os.path.dirname(__file__), f"JSONs/TriggerSF/2023postBPix/TriggerSF_sublead_2023postBPix_LowHgg_asym.json")
            logger.warning("Using 2023postBPix Trigger Efficiencies for 2024 as a placeholder until 2024 is ready available!")
        else:
        ...
        logger.warning("Using low mass H->gg SFs for this analysis!")
        json_file_lead = os.path.join(os.path.dirname(__file__), f"JSONs/TriggerSF/{year}/{year}_Lowmass_seeded_leg_efficinecy.json")
        json_file_sublead = os.path.join(os.path.dirname(__file__), f"JSONs/TriggerSF/{year}/{year}_Lowmass_unseeded_leg_efficinecy.json")

    lead_cs = correctionlib.CorrectionSet.from_file(json_file_lead)
    sublead_cs = correctionlib.CorrectionSet.from_file(json_file_sublead) # Reading JSON files

    
    evaluator_lead = lead_cs["HLT_2023_postBpix_normal_seed"] # Reading the correct sections from the JSON
    evaluator_sublead = sublead_cs["HLT_2023_postBpix_normal_unseed"]

    photon_pt = {  #making the variables to be used for trigger calculation
        1: photons["pho_lead"].pt,
        2: photons["pho_sublead"].pt,
        3: photons["pho_subsublead"].pt,
        4: photons["pho_subsubsublead"].pt,
    }
    

    photon_eta = {
        1: np.abs(photons["pho_lead"].ScEta),
        2: np.abs(photons["pho_sublead"].ScEta),
        3: np.abs(photons["pho_subsublead"].ScEta),
        4: np.abs(photons["pho_subsubsublead"].ScEta),
    }
    
    photon_r9 = {
        1: photons["pho_lead"].r9,
        2: photons["pho_sublead"].r9,
        3: photons["pho_subsublead"].r9,
        4: photons["pho_subsubsublead"].r9,
    }

    #logger.warning("testing inputs")
    #logger.warning(photons["pho_subsubsublead"].ScEta)

    def category_name(eta, r9): #finding out which string to be used (finding the eta,r9 bin string name)

        eta = np.abs(eta)
    
        cat = np.full(len(eta), "invalid",dtype=object)
        #logger.warning("during initialization")
        #logger.warning(cat)
        eb = eta < 1.5
    
        cat[eb & (r9 > 0.50) & (r9 < 0.55)] = "EB_R9_0p50to0p55"
        cat[eb & (r9 > 0.55) & (r9 < 0.85)] = "EB_R9_0p55to0p85"
        cat[eb & (r9 > 0.85)] = "EB_R9gt0p85"
    
        ee = eta >= 1.5
    
        cat[ee & (r9 > 0.90) & (r9 < 0.93)] = "EE_R9_0p90to0p93"
        cat[ee & (r9 > 0.93)] = "EE_R9gt0p93"
        #logger.warning("insid function")
        #logger.warning(cat)
        return cat
    ...
    for variation in ("nominal", "up", "down"):
        ...
        photon_cache[variation] = {}
    
        for ipho in range(1, 5):
    
            cats = category_name(
                photon_eta[ipho],
                photon_r9[ipho],
            )
            #logger.warning(cats[0])
            es = np.zeros(len(cats))
            eu = np.zeros(len(cats))
            ...
            if np.any(valid):
                ...
                for cat in categories:
                    ...
                    #logger.warning(cat)
                    #logger.warning(cats[:10])
                    mask = (cats == cat)
                    #logger.warning(mask)
                    #logger.warning(np.all(mask==False))
                
                    if not np.any(mask):
                        continue
                
                    es[mask] = evaluator_lead.evaluate(
                        variation,
                        cat,
                        photon_pt[ipho][mask],
                    )
                
                    eu[mask] = evaluator_sublead.evaluate(
                        variation,
                        cat,
                        photon_pt[ipho][mask],
                    )
    
            photon_cache[variation][ipho] = (es, eu)
            #logger.warning("es and eu values")
            #logger.warning(es[:10])
            #logger.warning(eu[:10])

    def compute_efficiency(cache):

        eff = np.zeros(len(weights._weight))

        for term in FINAL_TRIGGER_TERMS:

            prod = np.ones(len(weights._weight))

            for ipho, state in enumerate(term, start=1):
            ...
            eff += prod
            #logger.warning(f"term : {term}, prod : {prod[38]}")
        #logger.warning(f"final eff : {eff[38]}")
        return eff
        
    eff_nom = compute_efficiency(photon_cache["nominal"])
    zero_idx = np.where(eff_nom == 0)[0]
    
    #logger.warning(f"Number of zero-efficiency events = {len(zero_idx)}")
    """
    with open("zero_efficiency_events.txt", "w") as fout:
    
        fout.write(f"Number of zero-efficiency events = {len(zero_idx)}\n\n")
    
        for idx in zero_idx:
    
            fout.write("=" * 100 + "\n")
            fout.write(f"Event index : {idx}\n")
            fout.write(f"Efficiency  : {eff_nom[idx]}\n\n")
            ...
            for ipho in range(1, 5):
                ...
                es, eu = photon_cache["nominal"][ipho]
    
                fout.write(f"Photon {ipho}\n")
                fout.write(f"  pt     = {photon_pt[ipho][idx]}\n")
                fout.write(f"  eta    = {photon_eta[ipho][idx]}\n")
                fout.write(f"  r9     = {photon_r9[ipho][idx]}\n")
                fout.write(f"  cat    = {cats[idx]}\n")
                fout.write(f"  es     = {es[idx]}\n")
                fout.write(f"  eu     = {eu[idx]}\n")
                fout.write(f"  eu-es  = {eu[idx]-es[idx]}\n")
                fout.write(f"  1-eu   = {1.0-eu[idx]}\n\n")
                """
    if is_correction:

        # Only calculate correction to the nominal weight
        sf = eff_nom
        sfup = None
        sfdown = None
        ...
        # Only calculate systematic variations
        eff_up = compute_efficiency(photon_cache["up"])
        eff_down = compute_efficiency(photon_cache["down"])

        sf = np.ones(len(weights._weight))

        # Relative variations w.r.t. nominal
        sfup = np.ones(len(eff_nom))
        sfdown = np.ones(len(eff_nom)) # initializing arrays to one to account for division by zero due to events with 0 weight
        
        mask = eff_nom > 0
        
        sfup[mask] = eff_up[mask] / eff_nom[mask]
        sfdown[mask] = eff_down[mask] / eff_nom[mask]
        #sfup = eff_up / eff_nom
        #sfdown = eff_down / eff_nom
    ...
    '''
    logger.warning("=" * 100)
    logger.warning("CUSTOM TRIGGER EFFICIENCY CALCULATED SUCCESSFULLY")
    #logger.warning(list(eff_nom[:10]))
    logger.warning(len(np.where(eff_nom == 0)))
    test_idx = np.where(eff_nom == 0)[0][0]
    logger.warning("test index values")
    logger.warning(np.where(eff_nom == 0))
    logger.warning(np.where(eff_nom == 0)[0])
    logger.warning("testing values at problematic index")
    #logger.warning(ak.type(photons['pho_lead']))
    #logger.warning(type(photons['pho_lead'].pt))
    logger.warning(f"test_idx = {test_idx}")
    logger.warning(f"pt pho1 : {photons['pho_lead'][test_idx].pt} and pho2 {photons['pho_sublead'][test_idx].pt} and pho3 {photons['pho_subsublead'][test_idx].pt} and pho4 {photons['pho_subsubsublead'][test_idx].pt}")
    logger.warning(f"r9 pho1 : {photons['pho_lead'][test_idx].r9} and pho2 {photons['pho_sublead'][test_idx].r9} and pho3 {photons['pho_subsublead'][test_idx].r9} and pho4 {photons['pho_subsubsublead'][test_idx].r9}")
    logger.warning(f"ScEta pho1 : {photons['pho_lead'][test_idx].ScEta} and pho2 {photons['pho_sublead'][test_idx].ScEta} and pho3 {photons['pho_subsublead'][test_idx].ScEta} and pho4 {photons['pho_subsubsublead'][test_idx].ScEta}")
    #logger.warning(es[47])
    #logger.warning(eu[47])
    '''
    
    return weights

""" Original TriggerSF function
def TriggerSF(photons, weights, year="2017", is_correction=True, **kwargs):
...
def photonSFs(
    ...
    **kwargs,
    ...
    """
    Photon identification or reconstruction scale factors for Run 3 (2022/2023).
    Documentation: https://twiki.cern.ch/twiki/bin/view/CMS/EgammSFandSSRun3
    Returns event-level weights. The SF corresponds to the product over all photon SFs in the event.
    Take note that this is only correct if that number of photons is required in the event selection.
    ...
    #NOTE: This is in event_weight_corrections, so it is applied AFTER the full event sel. including 4 photon requirement.
    ...
    avail_years = ["2022preEE", "2022postEE", "2023preBPix", "2023postBPix", "2024"]
    if year not in avail_years:
        logger.error(f"Only photon corrections for {avail_years} implemented!")
        raise ValueError(f"Year '{year}' not supported for photon corrections.")

    path_json = os.path.join(os.path.dirname(__file__), f"JSONs/POG/EGM/{year}/photon.json.gz")
    evaluator = correctionlib.CorrectionSet.from_file(path_json)["Photon-ID-SF"]
    era_label = {
    ...
    def _eval(var_key: str, key: str, eta_in, pt_in):
        """Evaluate a single correction key (ID WP) on flat arrays."""
        # Weights for pT < 20 are split. Need a mask
        mask_lt20 = pt < 20
        mask_ge20 = pt >= 20

        sf_lt20 = evaluator.evaluate(era_label, var_key, f"{key}Below20", eta_in[mask_lt20], pt_in[mask_lt20])
        sf_ge20 = evaluator.evaluate(era_label, var_key, key, eta_in[mask_ge20], pt_in[mask_ge20])

        sf = np.ones(len(pt), dtype=float)
        sf[np.asarray(mask_lt20)] = ak.to_numpy(sf_lt20)
        sf[np.asarray(mask_ge20)] = ak.to_numpy(sf_ge20)

        return ak.Array(sf)

    # Event-level weights (product over photons)
    sf_nom = ak.unflatten(_eval("sf", sf_key, eta, pt), counts)  # N photons (counts) should = 4
    prod_nom = ak.prod(sf_nom, axis=1)

    if is_correction:
        name = f"PhotonId{sf_key}SF_corr"
        weights.add(name=name, weight=prod_nom, weightUp=None, weightDown=None)
    else:
        sf_up = ak.unflatten(_eval("sfup", sf_key, eta, pt), counts)
        sf_dn = ak.unflatten(_eval("sfdown", sf_key, eta, pt), counts)
        prod_up = ak.prod(sf_up, axis=1)
        prod_dn = ak.prod(sf_dn, axis=1)
        name = f"PhotonId{sf_key}SF"
