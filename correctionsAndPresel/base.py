...
from selections.photon_selections import photon_preselection_h4g, addRhoCorrections
...
class HggBaseProcessor(processor.ProcessorABC):  # type: ignore
    def __init__(
        ...
        h4g_bdt_dir = os.path.join(os.path.dirname(__file__), "../metaconditions/h4g/")
    ...
    def process(self, events: awkward.Array) -> Dict[Any, Any]:
        ...
        for variation in variations:
            ...
            diphotons = awkward.combinations(
                photons, 4, fields=["pho_lead", "pho_sublead", "pho_subsublead", "pho_subsubsublead"]
            )
            ...
            if self.h4g_bdt is not None:
                def predictBDT(
                    ...
                    '''
                    logger.warning("=" * 80)
                    logger.warning("BDT DEBUG")
                    logger.warning(f"Type of BDT object: {type(bdt)}")
                    
                    try:
                        logger.warning(f"n_features_in_: {bdt.n_features_in_}")
                    except Exception as e:
                        logger.warning(f"Could not get n_features_in_: {e}")
                    
                    try:
                        logger.warning(f"Booster num_features(): {bdt.get_booster().num_features()}")
                    except Exception as e:
                        logger.warning(f"Could not get num_features(): {e}")
                    
                    try:
                        logger.warning(f"Booster feature_names: {bdt.get_booster().feature_names}")
                    except Exception as e:
                        logger.warning(f"Could not get feature_names: {e}")
                    
                    logger.warning(f"DataFrame columns ({len(features.columns)}): {list(features.columns)}")
                    logger.warning(f"DataFrame shape: {features.shape}")
                    
                    if hasattr(ps, "m_hyp"):
                        logger.warning(f"m_hyp (first 10): {ps.m_hyp.to_numpy().flatten()[:10]}")
                    else:
                        logger.warning("m_hyp NOT found in ps")
                    
                    logger.warning("First event feature values:")
                    for col in features.columns:
                        logger.warning(f"  {col:25s}: {features[col].iloc[0]}")
                    
                    try:
                        dump = bdt.get_booster().get_dump(with_stats=False)
                        logger.warning("First 30 lines of first tree:")
                        for line in dump[0].split("\n")[:30]:
                            logger.warning(line)
                    except Exception as e:
                        logger.warning(f"Could not dump trees: {e}")
                    
                    logger.warning("=" * 80)
                    logger.warning(self.h4g_bdt.get_booster().feature_names)
                    '''
